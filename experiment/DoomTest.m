...
% Elasticity Test
function [ITERS, RESID, OC] = DoomTest(NLIST, stretch, EXPLIST, agg_rule)

  SetHomeDir

  dim=2;
  DOOM_ROOT_DIR=[MUEMAT_ROOT_DIR '/data/x_of_doom'];
  if ~varexist('NLIST')
    %NLIST=[0,1,2,3,4,5,6,7];
    %NLIST=[0,1,2,3,4];
    NLIST=[6];
  end

if ~varexist('stretch'),       stretch = 1;       end % default = no stretching
if ~varexist('EXPLIST'),       EXPLIST = {'sa', 'emin', 'ml-sa'}; end % default = run all
...
for I = 1:length(NLIST)
    ...
    %filename = ['doom.r' nstr '.mat'];
    %fprintf('Generating matrix (%s)', filename);
    CWD=pwd;
    cd(DOOM_ROOT_DIR);
    loadDataStr = ['[A,mesh] = tester(' nstr ',' num2str(stretch) ',false);'];
    eval(loadDataStr);
    %mesh=read_mesh(filename);
    %mesh.node(:,1)=stretch*mesh.node(:,1);
    %[K]=assemble_matrices(mesh);
    %IIDX=setdiff(1:mesh.Nn,mesh.NBIDX);
    %A=K(IIDX,IIDX);
    cd(CWD);
