...
%
% This function is a matrix-vector product with a block diagonal matrix where
% the blocks have a constant size (HasConstBlkSize) or have
% different sizes (HasVariableBlkSize).
%
% There is also a possibility of performing the matrix-vector product only on a
...
% format).
%
% When a matrix-vector product is performed only involving a subset
...
%
%  For (ConstBlkSize == 1), the simplier subfunction DNoBlkApply is called.
%
% Loop Variables:
% --------------------------------------------------------------------------
...
function [OutVec] = DBlkApply(A, Vec, Subset, VecType)
   ...
   if RowMap.HasConstBlkSize() && (RowMap.ConstBlkSize() == 1)
     OutVec = DNoBlkApply(A, Vec, Subset, VecType); %optim
   else
     ...
     if RowMap.HasConstBlkSize()
       ...
       for i=BlkList;
         ...
         OutVecPart = DSubFuncBlkApply(Amat, Vec, [fGlobalRow fLocalRow], [lGlobalRow lLocalRow], BlkSize, VecType);
...
%    contiguously and the output vector length is the number of rows in Subset
% row indices of Vec depends on VecType (case 1=GLOBAL, case 2=LOCAL).
function [OutVecPart] = DSubFuncBlkApply(M, Vec, fRow, lRow, BlkSize, VecType)
   GLOBAL = 1; % defined by mue_include. Inclusion of mue_include is slow
...
% if ConstBlkSize == 1
function [OutVec] = DNoBlkApply(A, Vec, Subset, VecType)
    GLOBAL = 1; % defined by mue_include. Inclusion of mue_include is slow
    if ~varexist('Subset'), Subset = []; end
    if ~varexist('VecType'), VecType = GLOBAL; end
