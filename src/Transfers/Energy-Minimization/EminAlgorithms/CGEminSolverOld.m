classdef CGEminSolver < VerboseObject
  ...
  properties (Access=private)
    nIts_ = 2
    SatisfyConstraintsFunc_ = @EminPFactory_.SatisfyConstraints
  end
  ...
  methods
    ...
    function P = Iterate(this, A, SparsityPattern, B, P0, CoarseLevel)
      P = this.Iterate_(A.GetMatrixData(), SparsityPattern, B, P0.GetMatrixData(), CoarseLevel);
    ...
    function P = Iterate_(this, A, SparsityPattern, B, P0, CoarseLevel)
      ...
      rk = this.SatisfyConstraintsFunc_(-A*P, [], CoarseLevel);
      ...
      while ( (i<=this.nIts_) && (max(max(abs(rk)))>1e-12))
        ...
        ap = this.SatisfyConstraintsFunc_(A*pk, [], CoarseLevel);
