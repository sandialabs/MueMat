classdef GMRESEminSolver < VerboseObject
    properties (Access = private)
        nIts_ = 2;
        %% TODO: make use of SatisfyConstraintsFunc_!
        SatisfyConstraintsFunc_ = @EminPFactory.SatisfyConstraints
    end

    methods
        function this = GMRESEminSolver(nIts)
            if nargin == 1 && isa(nIts, class(this)), this.Copy_(nIts,[]); end
...
%             CoarseLevel.Request('Ppattern',PatternFact);
        end
        ...
        function P = Iterate_(this, A, SparsityPattern, B, P0, CoarseLevel)
            %
            % initialization
            %
            n = size(A,1);
            ...
            while(k < Nits)
                ...
                %
                if k > 1
                    h(1:k,k)=this.givapp(c(1:k-1),s(1:k-1),h(1:k,k),k-1);
                end
                nu=norm(h(k:k+1,k));
                if nu~=0
                    ...
                    h(k+1,k)=0;
                    g(k:k+1)=this.givapp(c(k),s(k),g(k:k+1),1);
                end
                %
    ...
    methods(Access = private)
        function vrot=givapp(this,c,s,vin,k)
