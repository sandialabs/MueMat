...
classdef Scatra2D
   ...
   methods (Access = public)
       function [this] = Scatra2D(nele,kappa,frhs,afun,description)

           load geometry3.mat      % cube [-1,1]^2
           [p,e,t] = Scatra2D.poi_mesh(g,nele,nele);
	   %load mesh32.mat;
           ...
           this.problem_.bc.dirichnodes = find(p(1,:)==-1 | p(2,:)==-1 | p(1,:)==1);             % left, bottom and right border
           ...
           else this.problem_.afun = inline('[2*y*(1-x*x),-2*x*(1-y*y)]','x','y'); end;
