...
function [Amat] = BuildLaplaceND(NPTS)
      newMap = Map(prod(NPTS), 1);
      MatrixData = BuildMatrixLaplaceND(NPTS);

      Amat = Operator(MatrixData,newMap,newMap,@MatlabApply, ['Laplace' num2str(length(NPTS)) 'D']);
end
...
function [MAT,NODES,DN]=BuildMatrixLaplaceND(NPTS,varargin)
