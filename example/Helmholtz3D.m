...
% Build a 3D Helmholtz problem
waves=5; npts=10; medium=0; N=waves*npts;
lambda=1/waves;
omega=2*pi/lambda;
[Kmat, Mmat, Nullspace, Nx, Ny, Nz] = BuildHelmholtz3D(waves,npts,medium);
...
b    = zeros(nDOFS,1);
h    = lambda/npts;
idx  = reshape(1:nDOFS,N,N,N);
b(idx(end/2,end/2,end/2)) = 1/(h*h);
%[x, flag, relRes, nIts, resVec] = gmres(A.GetMatrixData(),b,restart,tol,maxIts,@(b)MgHierarchy.Iterate(b,1, zero,ALLZEROS));
[x, flag, relRes, nIts, resVec]  = bicgstab(A.GetMatrixData(),b,tol,maxIts,@(b)MgHierarchy.Iterate(b,1, zero,ALLZEROS));
fprintf('converged to the desired tolerance %g within %d iterations.\n', tol, nIts);
xyplane=idx(:,:,end/2);
xi=x(xyplane(:));
xi=reshape(xi,N,N);
imagesc(real(xi)); colorbar; axis equal; axis tight;
